    ...
    public function store(StoreEventEvaluationRequest $request, Event $event): JsonResponse
    {
        ...
        $registration = Registration::query()
            ...
            ->where(function ($query) use ($email) {
                ...
                })->orWhereHas('eventRegistrant', function ($registrantQuery) use ($email) {
                    $registrantQuery->whereRaw('LOWER(email) = ?', [$email]);
                });
            })
        ...
        $participant = $registration->user_id !== null
        ...
        if (! $hasEventId || ! $hasRegistrationId) {
            if ($participant === null) {
                throw ValidationException::withMessages([
                    'email' => 'Evaluation storage is using the legacy schema and requires a participant user account. Please apply the latest evaluations migration to support event-based evaluations.',
                ]);
            }
